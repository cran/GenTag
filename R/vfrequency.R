vfrequency <-
function(ntag,colorsname,colorsf,nspecial=0, name1="Metal",name2="EMPTY", location1=1, location2=2,nspecial1=1,nspecial2=1){
if(nspecial>2){stop("Maximum of 2 special band allowed")}
colorsname<-as.vector(colorsname)
if(nspecial==0){amostra<-sample(x=colorsname,size=ntag,replace=TRUE,prob=colorsf)}else{
amostra<-rep(NA,ntag)
...
amostra[para_pintar]<-sample(x=colorsname,size=length(para_pintar),replace=TRUE,prob=colorsf)}
